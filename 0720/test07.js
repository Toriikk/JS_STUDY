...
function promiseForHomework(mustDo) { // mustDo == true
    // Promise 객체는 Promise 생성자 함수에 new 키워드를 통해 생성할 수 있다.
    return new Promise((resolve, reject) => { // 콜백 함수 선언하고, resolve reject를 인자로 줌 
        setTimeout(() => {
...
promiseA.then((v) => console.log(v)); // then 통해서 처리 결과값 받기
promiseB.then((v) => console.log(v)).catch((e) => console.log(e)); // then 통해서 처리 결과값 받기